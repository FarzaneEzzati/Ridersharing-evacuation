...
for i in N_2:
    dm[i] = 1.0
dm[10], dm[11], dm[13], dm[16],  = 2, 1, 2, 2
...
cv[10], cv[11], cv[13], cv[16], cv[19]  = 2, 5, 5, 3, 100
...
w_2 = 0.0001
#### value store here ####
x_var = np.zeros((num, num, num, scenario))  # x_i,j^v,m
y_var = np.zeros((num, num))  # q_i^v
r_var = np.zeros((num, num))  # r_i^v
...
class model:
    ...
    def createmodel(self):
        self.mdl.minimize(self.mdl.sum(self.x[0, j, k, m] for j in N_2 for k in Vh for m in scset) +
                          w_2 * self.mdl.sum(self.r[i, k] + self.y[i, k] for i in N for k in Vh) +
                          w_s1 * self.mdl.sum(self.x[0, sd, sd, m] for m in scset) +
                          w_s2 * self.mdl.sum(
            self.x[i, j, sd, m] * tt[i][j][m] for i in N for j in N if i != j for m in scset))
        # constriant 2
        for i in N_2:
            self.mdl.add_constraint(self.mdl.sum(self.x[0, i, i, m] for m in scset) - self.mdl.sum(
                self.x[N_D[i], N[-1], i, m] for m in scset) <= 0)
        ...
        self.mdl.add_constraints(
            self.mdl.sum(self.x[i, j, k, m] for m in scset) <= 1 for i in N for j in N if j != i for k in Vh)
        # constraint 8
        for i in N_1:
            self.mdl.add_constraint(
                self.mdl.sum(self.x[i, j, k, m] for j in N if j != i for k in Vh for m in scset) == 1)
        for i in N_2:
            self.mdl.add_constraint(
                self.mdl.sum(self.x[i, j, k, m] for j in N if j != i for k in Vh for m in scset) == 1)
        for i in N_3:
        ...
        for i in N_0:
            for k in Vh:
                self.mdl.add_constraint(
                    self.mdl.sum(self.x[i, j, k, m] for j in N if j != i for m in scset) - self.mdl.sum(
                        self.x[j, i, k, m] for j in N if j != i for m in scset) == 0)
        # constraint 10
        for k in Vh:
            for i in N_2:
                self.mdl.add_constraint(
                    self.mdl.sum(self.x[j, i, k, m] for j in N if j != i for m in scset) - self.mdl.sum(
                        self.x[j, N_D[i], k, m] for j in N if j != N_D[i] for m in scset) <= 0)
            for i in N_3:
                self.mdl.add_constraint(
                    self.mdl.sum(self.x[j, i, k, m] for j in N if j != i for m in scset) - self.mdl.sum(
                        self.x[j, N_D[i], k, m] for j in N if j != N_D[i] for m in scset) <= 0)
        ...
        self.mdl.add_constraints(
            ...
            k in N_1 for m in scset)
        self.mdl.add_constraints(
            self.r[j, k] - self.r[i, k] - M * self.x[i, j, k, m] >= tt[i][j][m] - M for i in N for j in N if j != i for
            k in N_2 for m in scset)
        self.mdl.add_constraints(
            self.r[j, k] - self.r[i, k] - M * self.x[i, j, k, m] >= tt[i][j][m] - M for i in N for j in N if j != i for
            k in N_3 for m in scset)
        # constraint 12
        self.mdl.add_constraints(
            self.r[j, k] + M * self.x[i, j, k, m] <= M + sec_T[m] for i in N for j in N if j != i for k in Vh for m in
            scset)
        # constraint 13
        self.mdl.add_constraints(
            self.r[j, sd] - self.r[i, sd] - M * self.x[i, j, sd, m] >= tts[i][j][m] - M for i in N for j in N if j != i
            for m in scset)
        # constraint 14
        self.mdl.add_constraints(
            self.y[j, k] - self.y[i, k] - M * self.mdl.sum(self.x[i, j, k, m] for m in scset) >= dm[j] - M for i in N
            for j in N if j != i for k in Vh)
        # constraint 15
